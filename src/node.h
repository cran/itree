...
struct split {
    ...
    int var_num;   /* the number of the predictor used to make the split ( 0 <= var_num <= (p-1)) */
    int count;
    int csplit[2];     /*the actual length will be longer for a categorical */
    };                 /*   predictor with >2 levels */

struct node {
    double  risk;       /*risk for the node ALG 1/24/2012: this is ss for regression*/
    double complexity;  /* complexity at which it will collapse */
    double sum_wt;      /* sum of the weights for the node  */
    struct split *primary;  /*split object for primary variable*/
...
EXTERN int nodesize;  /*ALG: this is size in memory, not number of observations */
