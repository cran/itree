 ...
 * Here we just call the eval fcn
 */
void poisson_parent_objective(int n, double *y[], double *value, double *parent_objective,
	     double *wt) {
	poissondev(n,y,value,parent_objective,wt);
}
...
void poisson(int n,       double **y,      FLOAT *x,     int nclass, 
	     ...
	     int *csplit, double my_risk,  double *wt, double *parent_objective)
    {
    int i,j;
    int    left_n, right_n;
